...
| 47 | 集成测试手写 schema 与生产迁移链形成双重真相 | 🗄️ 数据 | ✅ |
...
## 47. 集成测试手写 schema 与生产迁移链形成双重真相

**现象**：`testbug/helpers.ts` 维护了一份手写 SQLite 建表脚本。生产迁移新增字段或表后，集成测试仍可能在旧结构上通过，无法证明真实旧库可以安全升级。

**根因**：测试环境把数据库结构复制成第二份实现，既绕过 `drizzle/migrations`，清理逻辑又写死四张旧表，因此迁移与测试会独立演化。

**修复**：

- 集成测试统一执行生产 `drizzle/migrations` 建库。
- 清理逻辑从 `sqlite_master` 动态发现应用表，保留迁移记录并执行外键完整性检查。
- 固化 `pre-social-0006.sql` 历史快照，覆盖旧用户字段、匿名内容、两层回复、点赞与收藏；验证升级、重复执行迁移和关闭后备份读取。

**预防**：数据库结构只有生产迁移链一个真相来源；历史 fixture 合入后不可改写，新的兼容边界必须新增 fixture，并在迁移测试中验证完整性、外键与数据保留。
