...
12. **迁移单一真相**：集成测试从真实 `drizzle/migrations` 建库；历史迁移 fixture 合入后不可追随最新 schema 改写，新的兼容边界用新 fixture 表达
13. **访问策略前置**：所有消息读取面复用中央 SQL predicate，并在计数、排序、分页、搜索和聚合前完成授权；禁止取回后再用 JavaScript 过滤
